...
static const char *strDNSSeed[][2] = {
    // {"gigacoinpool.org", "dnsseed.gigacoinpool.org"},
...
unsigned int pnSeed[] =
{
	0xdddb0dad, 0x5014ea36, 0xe4181617, 0xf43bc497,
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Gigacoin is probably already running."), addrBind.ToString().c_str());
        else
